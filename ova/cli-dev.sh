...
ensure_timescale_preload_config() {
  ...
  local pg_conf="${INSTALL_DIR}/${POSTGRES_DATA_DIR}/data/postgresql.conf"
  local changed=false
  ...
  # Keep TimescaleDB preloaded for existing appliances. Older bad bundles
  # stripped it from the compose command, which overrides postgresql.conf and
  # causes "extension timescaledb must be preloaded" during migrations.
...
update() {
  ...
  if [ -f "$TEMP_FILE" ]; then
    ...
    if [ "$current_swapfile_gb" -eq "$swapfile_target_gb" ]; then
      echo "[OK] Swap is ${current_total_gb}GB (RAM: ${total_ram_gb}GB, target: ${target_swap_gb}GB, swapfile: ${current_swapfile_gb}GB)"
    else
