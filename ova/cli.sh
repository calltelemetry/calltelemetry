...
update() {
  ...
  if [ -f "$TEMP_FILE" ]; then
    ...
    ensure_timescale_preload_config "$ORIGINAL_FILE"
    ...
    if ! verify_timescale_runtime_preload; then
      rm -f "$caddyfile_tmp"
      rm -f "${INSTALL_DIR}/.ssh/authorized_keys"
      return 1
    fi
...
check_database_ready() {
  local container health output rc

  container=$($DOCKER_COMPOSE_CMD ps -q db 2>/dev/null)
  if [ -z "$container" ]; then
  ...
  if [ "$health" = "unhealthy" ]; then
    echo "Docker healthcheck reports db is unhealthy"
    return 2
  fi
  ...
  output=$($DOCKER_COMPOSE_CMD exec -e PGPASSWORD=postgres -T db psql \
    -v ON_ERROR_STOP=1 \
    -U calltelemetry \
    -d calltelemetry_prod \
    -tAc "SELECT 1; CREATE TEMP TABLE ct_cli_db_healthcheck(id integer); DROP TABLE ct_cli_db_healthcheck;" 2>&1)
  rc=$?

  if [ $rc -ne 0 ]; then
    if printf '%s\n' "$output" | grep -qiE 'timescaledb.*must be preloaded|shared_preload_libraries|could not access file.*timescaledb'; then
      echo "Database accepts connections, but TimescaleDB is not usable:"
      printf '%s\n' "$output" | sed 's/^/    /'
      echo "DDL probe failed; migrations will not run correctly until TimescaleDB config/extension state is repaired."
      return 2
    fi

    echo "Database accepts connections, but SQL/DDL readiness probe failed:"
    printf '%s\n' "$output" | sed 's/^/    /'
    return 2
  fi
...
wait_for_services() {
  ...
  # Phase 2: Wait for database to accept connections and prove DDL works
  echo "Phase 2: Waiting for database..."
  local db_ok=false
  local db_terminal_failure=false
  local db_check_output=""
  local db_check_rc=0
  wait_time=0
  while [ $wait_time -lt 120 ]; do
    db_check_output=$(check_database_ready 2>&1)
    db_check_rc=$?

    if [ $db_check_rc -eq 0 ]; then
      echo "  ✓ Database accepting connections and SQL/DDL ready"
      db_ok=true
      break
    elif [ $db_check_rc -eq 2 ]; then
      echo ""
      echo "  [FAIL] Database readiness check failed:"
      printf '%s\n' "$db_check_output" | sed 's/^/    /'
      db_terminal_failure=true
      break
    fi
  ...
  if [ "$db_ok" != true ]; then
    echo ""
    if [ "$db_terminal_failure" = true ]; then
      echo "  [FAIL] Database is not healthy enough to run migrations"
    else
      echo "  [FAIL] Database connection timed out after 120s"
    fi
    echo "  Skipping migration monitoring because database readiness failed."
    phase_failures=$((phase_failures + 1))
    ...
    echo "    cli.sh logs db --tail 80   Recent database logs"
    echo "    cli.sh logs web --tail 80  Recent web logs"
    return 1
  fi
  echo ""
  ...
  local health_timeout="${CT_STARTUP_HEALTH_TIMEOUT:-180}"
  local rpc_timeout="${CT_STARTUP_RPC_TIMEOUT:-180}"
  if ! [[ "$health_timeout" =~ ^[0-9]+$ ]] || [ "$health_timeout" -lt 20 ]; then
    health_timeout=180
  fi
  if ! [[ "$rpc_timeout" =~ ^[0-9]+$ ]] || [ "$rpc_timeout" -lt 20 ]; then
    rpc_timeout=180
  fi
  ...
  while [ $health_elapsed -lt $health_timeout ]; do
    ...
    printf "\r  Web health: waiting... (%ds/%ds)" "$health_elapsed" "$health_timeout"
    sleep 3
    health_elapsed=$((health_elapsed + 3))
  done
  [ "$web_healthy" = true ] && printf "\r"

  if [ "$web_healthy" = true ]; then
    echo "  ✓ Web application healthy"
  else
    echo ""
    echo "  [FAIL] Web health check failed after ${health_timeout}s"
    phase_failures=$((phase_failures + 1))
  ...
  local rpc_output=""
  local rpc_elapsed=0
  while [ $rpc_elapsed -lt $rpc_timeout ]; do
    rpc_output=$($DOCKER_COMPOSE_CMD exec -T web "$release_bin" rpc 'IO.puts("ok")' 2>&1)
    if [[ "$rpc_output" == *"ok"* ]]; then
      rpc_healthy=true
      break
    fi
    printf "\r  Application RPC: waiting... (%ds/%ds)" "$rpc_elapsed" "$rpc_timeout"
    sleep 3
    rpc_elapsed=$((rpc_elapsed + 3))
  done
  [ "$rpc_healthy" = true ] && printf "\r"

  if [ "$rpc_healthy" = true ]; then
    echo "  ✓ Application RPC responding"
  else
    echo ""
    echo "  [FAIL] Application RPC not responding after ${rpc_timeout}s"
    if [ -n "$rpc_output" ]; then
      printf '%s\n' "$rpc_output" | tail -5 | sed 's/^/    /'
    fi
    phase_failures=$((phase_failures + 1))
      ...
      profile)
        ...
        case "$subaction" in
          show)
            ...
            echo ""
            echo "  small  — 100 max connections, 15 per pool"
            echo "  medium — 200 max connections, 20 per pool"
            echo "  large  — 300 max connections, 25 per pool"
            echo ""
            echo "Current values:"
